...
frontend/
  ...
  src/
    ...
    components/
      ...
      DocumentEditor.tsx     - Auto-saving Milkdown Crepe editor with a visible manual save button, lightweight draft creation for new notes, and lightweight PUT updates for existing notes
...
- **DocumentEditor** (`DocumentEditor.tsx`) - Auto-saving Milkdown Crepe editor. Reuses the same Crepe configuration as the legacy NoteEditor (ProseMirror WYSIWYG with LaTeX support). On every content or title change, a 1.5-second debounce timer starts. When it fires, new documents POST to `/api/documents` for a fast lightweight draft save and existing documents PUT to `/api/documents/{docId}`. The header also keeps a visible manual `Save note` button so the save action is always present even before auto-save fires. Title-only drafts are allowed, and pending saves flush immediately on unmount.
