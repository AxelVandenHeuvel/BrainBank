...
frontend/
  ...
  src/
    ...
    components/
      ...
      Graph3D.tsx            - 3D graph scene with right-side zoom/reset/brain-mesh controls, a soft pink brain wireframe shell that fades in and out, a temporary debug color picker with separate background and brain-mesh inputs for solid-color testing, dodecahedron nodes, force-directed layout, animated node-to-node focus transitions, concept dive-in with document sub-graph, and callback props (onOpenDocument, onConceptFocused) for parent tab integration
